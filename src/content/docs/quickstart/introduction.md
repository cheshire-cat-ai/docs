...
There are a few things you need to know about the Cheshire Cat. If you are eager to launch and start hacking, jump to the [installation page](/docs/quickstart/installation-configuration/), but please be sure to return here.
...
You can write Python functions called [`Tools`](/docs/plugins/tools/) and have the LLM execute this code.
...
In the main process flow, there are predefined adaptation points called [`Hooks`](/docs/plugins/hooks/).
You can write Python functions that can be attached onto these [`Hooks`](/docs/plugins/hooks/).
...
[`Tools`](/docs/plugins/tools/) and [`Hooks`](/docs/plugins/hooks/) are packaged into [`Plugins`](/docs/plugins/plugins/) that can be installed by placing files in a specific folder or using the `Admin Portal`.

The `Mad Hatter` is the component that manages plugins.

## Sharing Plugins
If desired, you can  [`publish your Plugins`](/docs/plugins/plugins-registry/publishing-plugin/) on the public registry.
...
## Next step
In the [next step](/docs/quickstart/installation-configuration/), you will learn how to install the Cat, set the LLM and the basics of this all.
...
Also, the Cat will be able to tell the price of socks according to the requested color (using a [`Tools`](/docs/plugins/tools/)).
In the end, we will transform the sock seller into a poetic socks seller, changing its personality (using a [`Hooks`](/docs/plugins/hooks/)). 
