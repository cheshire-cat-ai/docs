...
A minimal web UI is included in plugin `ui`, automatically installed at first launch. Find it at [localhost:1865](http://localhost:1865).  
...
To use the Cat from another application check out the endpoints:
 - point your coding agent to [localhost:1865/openapi.json](http://localhost:1865/openapi.json)
 - play around visually with the endpoints at [localhost:1865/docs](http://localhost:1865/docs)
...
- `{slug}` is the id of the agent you want to talk to. Every fresh install ships a built-in agent with slug `default`, so use `default` if you have not written your own agent yet. You can list every registered agent at [localhost:1865/agents](http://localhost:1865/agents).
