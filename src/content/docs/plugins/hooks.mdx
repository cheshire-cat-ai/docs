...
Not all the hooks have been documented yet. ( [help needed! &#128568;](https://discord.com/channels/1092359754917089350/1092360068269359206) ).
...
1. **Input arguments**  
    ...
    ??? example
        ...
        def activated(plugin):
            ...
            ccat.rabbit_hole.ingest_file(file=url)
        ```
...
To create a hook, you first need to create a [plugin](/docs/plugins/plugins/) that contains it. Once the plugin is created, you can insert hooks inside the plugin, a single plugin can contain multiple hooks.
...
- `cat` will always be present, as it allows you to use the framework components. It will be always the last one.
    ```python
    @hook
    def hook_name(cat):
        pass
